...
par_list = dir(fullfile(sess_path, 'Parfiles', '*par'));
for par = 1:numel(par_list)
    par_file{par} = fullfile(sess_path, 'Parfiles', par_list(par).name);
...
params.subject = subj;
%params.annotations = {'label 1', 'label2'};  %with this, you can annotate
%each epi/scans in the order that mrVista reads them here in the list created above
%params.scanGroups = {[1], [2, 3]}; %with this, you can group scans
%together for average when doing stats - in the exemple here, 1 is a group,
