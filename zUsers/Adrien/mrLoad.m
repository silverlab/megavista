...
if length(mr.voxelSize)<4
    warning('mr.voxelSize has length<4: concatenating a "1" to it')
    mr.voxelSize(end+1:4) = 1; 
end

if length(mr.dims)<4    
    warning('mr.dims has length<4: concatenating a "1" to it')
    mr.dims(end+1:4) = 1;     
end
...
try 
    ...
    disp('mrLoad error with minmax improperly used on a matrix >2D. Edited to use custom function minmaxAll instead - line 268. Adrien Chopin, 2015')
    mr.dataRange = minmaxAll(mr.data);
end
