...
try
	...
	% this alternate version only works on 32-bit machines
	warning(['[%s]: The MEX file "readFileNifti" is not working on this ' ...
             'MATLAB version. Trying a fallback, more stable version. ' ...
             'The data structure be the same, except the qform code may ' ...
             'not be properly read.'], mfilename);
	mr.hdr = readFileNifti_stable(pth);  
end

% 2011.07.26 RFD: apply the cannonical transform to handle niftis
% properly regardless of the data orientation.
disp('mrReadNifti: edited to avoid errors - Eunice Yang, Rachel Denison.')
if sum(mr.hdr.dim>100)==3 % only for mprage
    mr.hdr  = niftiApplyCannonicalXform(mr.hdr); %%% does not work for coronal slices (rd)
end
mr.data = mr.hdr.data;
