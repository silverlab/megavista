...
% do it
[params,d,d,exitflag,output] = lsqcurvefit(@(pp,xx)nanreplace(evalgaussian3d(copymatrix(params0,ix,pp),xx),0,2),params0(ix),[flattenIt(xx); flattenIt(yy); flattenIt(zz)],flattenIt(m),paramslb(ix),paramsub(ix),options);
assert(exitflag > 0);
params = copymatrix(params0,ix,params);

% how well did we do?
R2 = calccod(evalgaussian3d(params,[flattenIt(xx); flattenIt(yy); flattenIt(zz)]),flattenIt(m));
