...
try
     ...
     disp('rxRefresh error with non-structure array: edited line 29. Adrien Chopin, 2015')
    %the problem is that rx can output more than one structure array in a cell.
    %when one tries to reach rx.xform, this is now a cell array, not a structure with fields...
    %the solution chosen here is to focus on the last structure created.
    rx = rx{end};
    rx.prevXform = rx.xform;
end
