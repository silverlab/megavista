% rd_mpDistributionZ.m

%% Setup
hemi = 2;
scanDate = '20110920'; % CG: '20110128'; WC: '20110819','20110908','20100920'
...
%% File I/O
fileBase = sprintf('lgnROI%dAnalysis_%s_mpDistributionZ%s', hemi, scanDate, datestr(now,'yyyymmdd'));
...
%     annotation('ellipse',dsxy2figxy([-2 -2 4 4]))
    ...
    for cond = 1:numConditions
        figure
        superthreshInCond = zScores(superthreshZs(:,cond),:);
...
%         plot(zScores(superthreshZs(:,cond),:))
    ...
    ylabel(condNames{condsToPlot(2)})
    zlabel(condNames{condsToPlot(3)})
    title(sprintf('Hemi %d, zThresh = %.1f\nColor = %s', ...
        hemi, zThresh, ...
        condNames{condToColor}))
    ...
    ylabel(contrasts.zo(contrastsToPlot(2)).name)
    zlabel(contrasts.zo(contrastsToPlot(3)).name)
    title(sprintf('Hemi %d, zMeanThresh=%.1f\nColor = %s', ...
        hemi, zMeanThresh, ...
        contrasts.zo(contrastToColor).name))
