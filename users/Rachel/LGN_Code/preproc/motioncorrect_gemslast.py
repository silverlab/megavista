...
if __name__ == "__main__":
    ...
    for this_epi in epi_list[-1::-1]: # count backwards from end 
        print this_epi
        #In the last epi in the directory, convert the last image to nifti,
        #to be used in motion correction afterwards as reference:
        if this_epi == epi_list[-1]:
            print('Creating reference volume from last epi volume')
            epilast_dir = os.path.splitext(os.path.splitext(this_epi)[0])[0]
            dicom_names = os.listdir(os.path.join(dicom_dir, epilast_dir))
            last_dicom = dicom_names[-1]

            os.chdir(dicom_dir + epilast_dir)
            print os.path.realpath(os.path.curdir)

            os.system('dcm2nii -g N -v N {}'.format(last_dicom))
