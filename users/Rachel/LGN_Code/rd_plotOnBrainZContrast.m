% rd_plotOnBrainZContrast.m

%% Setup
hemi = 1;
scanDate = '20110805';
analysisDate = '20110805';
zContrast = 7;
...
analysisExtension = sprintf('_mpDistribution%s', analysisDate);
...
for iSlice = 1:length(slices)
    for iROI = 1
        ...
        vals = zContrasts(inSlice,zContrast);
...
for iSlice = 1:length(slices)
    for iROI = 1
        ...
        xycoords = inplaneCoords{iROI}(1:2,inSlice);
        vals = zContrasts(inSlice,zContrast); % choose val to show here
...
%% save map
mapSavePath = ...
    sprintf('mat_files/%s%s%s.mat', mapFileBase, contrasts.z(zContrast).name, datestr(now,'yyyymmdd'));
