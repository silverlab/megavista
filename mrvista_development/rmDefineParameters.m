function params = rmDefineParameters(view, varargin)
% rmDefineParameters - define parameters for retinotopic model fit
%
% params = rmDefineParameters([view], [varargin]);  
%
% inputs:
%  view :       mrVista struct
...
% degrees).
params.analysis.numberStimulusGridPoints = 20; % 50 %%%% changed by RD to 20
...
try
    ...
    params.analysis.dataType    = viewGet(view, 'dataTypeName');
...
params = rmGetStimType(view, params);
...
if params.analysis.coarseToFine,
    if prod(viewGet(view,'mmPerVox')) >= 1,
