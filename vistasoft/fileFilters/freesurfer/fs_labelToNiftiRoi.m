function [nRoi fsLabelName] = fs_labelToNiftiRoi(fsIn,labelVal,outName,smoothKernel)
...
% smoothKernel = 3 will smoothe the roi with a 3mm [3 3 3] kernel. This
% will produce 2 rois; one without smoothing and one with.
% 
...
%       fslabel.mat) It has all of the label numbers and names.
% 
...
%       Remove the warning dialog - and just have the warning throw to the
%       prompt. Could just take in a subject's name and take the fs sub dir
%       after that. Check to see if the labelVal exist in the fslabel.mat.
%        
% HISTORY: 
...
% Check that FSL is in the path
fsd = getenv('FSLDIR');

if ~exist(fsd,'file'); 
    error('You need FSL for this operation to work. Make sure FSL is in your shell path and restart matlab.'); 
end

% Set the file type to be nifti in FSL
!export FSLOUTPUTTYPE=NIFTI_GZ

% Get the fsIn file. If it's an mgz file we convert it to nifti.
...
% In the scripted case: If a name for the roi is not provided the label number is used and the
% path for the roi is set to the be the same as that of the seg file. 
if notDefined('outName')
    ...
    outName = [p '/roi_'  fsLabelName '_' labelVal  '_' n e];
end
...
% DO THE THING: Use fslmaths thresh to select the labels and make the nifti.
...
if flag == 1
   ...
   h = warndlg(sprintf('You have selected an mgz file. \n We will have to convert that mgz to nifti using fs_mgzSegToNifti...'),...
...
if isempty(inputs)
    ...
    if ~isempty(inputs{2})
        ...
        if check==0
            ...
            fsIn(inds)=[];
            [p n] = fileparts(fsIn);
            outName = [p '/roi_' uiName '_' n '.nii.gz'];
        else
            outName = uiName;
        end
    else
