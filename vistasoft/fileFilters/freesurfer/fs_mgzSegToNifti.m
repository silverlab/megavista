function fs_mgzSegToNifti(mgzIn, refImg, outName, orient)

% function fs_mgzSegToNifti(mgzIn,refImg,[outFileName],[orient])
% 
...
if notDefined('mgzIn') || ~exist(mgzIn,'file')
    error('You must provide the path to the mgz file you wish to convert to Nifti');
end

if notDefined('refImg') || ~exist(refImg,'file')
    [fname path] = uigetfile({'*.nii.gz'},...
        'Please select a T1 file for alighnment of the segemntation file...');
    refImg = [path fname];
end
