...
% functionals
defaultFuncPath = fullfile(params.sessionDir, 'Raw', 'Pfiles');
if exist(defaultFuncPath, 'dir')
    pattern = fullfile(defaultFuncPath, 'P*.7.mag');
    if ~isempty( dir(pattern) )
        params.functionals = mrInitMagFiles(defaultFuncPath);
    end
end

% volume anatomy
defaultVolAnatPath = fullfile(params.sessionDir, '3DAnatomy', 'vAnatomy.dat');
if exist(defaultVolAnatPath, 'file')
    params.vAnatomy = defaultVolAnatPath;
end
