function view = removeScan(view, scan, dt, delTSeries);
%
% view = removeScan(view, [scan], [dataType], [delTSeries]);
%
...
if notDefined('scan'), scan=getCurScan(view);         end
...
% proper name):
nScans = length(dataTYPES(dt).scanParams);
...
%% update GUI
if isfield(view, 'ui')
	if view.curDataType==dt & view.curScan > numScans(view)
		view = selectDataType(view, view.curDataType); % this resets scan slider
