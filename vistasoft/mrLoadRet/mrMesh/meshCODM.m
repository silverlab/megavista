function [dataMaskIndices,data,cmap,dataRange,phaseFlag] = meshCODM(view, clusterThresh, dataScale)
% Mesh Color Overlay / Data Mask function.
%
% [dataMaskIndices, dataOverlay, cmap, dataRange, phaseFlag] = meshCODM(view, clusterThresh, dataScale)
%
%
...
if viewGet(view, 'maskROIs')
    roiInd = roiGetAllIndices(view);
...
switch view.ui.displayMode
    case {'co','coherence'}
        if coOK
            ...
            data = co;
            dataRange = view.ui.coMode.clipMode;
    ...
    case 'amp'
        if coOK
            ...
            dataRange = view.ui.ampMode.clipMode;
			if isequal(dataRange, 'auto')
				dataRange = [mnv mxv];
			end
        end
...
if(clusterThresh>0)
    sz = viewSize(view);
    maskVol = false(sz);
    maskCoords = view.coords(:,dataMaskIndices);
