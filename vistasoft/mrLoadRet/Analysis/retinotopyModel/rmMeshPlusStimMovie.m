function [M imgCmap] = rmMeshPlusStimMovie(V, p, varargin);
...
%
%	[M imgCmap] = rmMeshPlusStimMovie(view, params);
%
% INPUTS:
%
%	view: gray view. Needs to have both a mesh loaded, and retiontopy model
...
def = rmMeshPlusStimMovie_defaultParams(V);

if notDefined('p')
	if isempty(varargin)
		p = rmMeshPlusStimMovie_paramsGUI(V, def);
	else
		p = def;
	end
else
...
if ~isempty(p.meanthresh) & p.meanthresh > 0
	...
	lowMean = find(V.map{scan} < p.meanthresh);
end

lowCo = [];
if ~isempty(p.cothresh) & p.cothresh > 0
	lowCo = find(V.co{scan} < p.cothresh);
end
...
V.ui.mapMode.cmap = [gray(128); p.cmap];
V.ui.mapMode.clipMode = p.clim;
...
% /---------------------------------------------------------------/ %
function p = rmMeshPlusStimMovie_defaultParams(V);
...
% /---------------------------------------------------------------/ %
function p = rmMeshPlusStimMovie_paramsGUI(V, def);
