...
try
    switch lower(param),
        ...
        %Measures for difference of gaussian pRF models, which make more sense than pRF sigma in these cases    
        case {'fwhm', 'sigma2fwhm','fwhmax', 'sigma2fwhmax'}
            [val, tmp1, tmp2, tmp3, tmp4, tmp5] = rmGetDoGFWHM(model); %#ok<*NASGU>
        case {'fwhmin', 'sigma2fwhmin'}
            [tmp1, val, tmp2, tmp3, tmp4, tmp5] = rmGetDoGFWHM(model);             %#ok<*ASGLU>
        case {'sigma2minima', 'sigma2min'}
            [tmp1, tmp2, val, tmp3, tmp4, tmp5] = rmGetDoGFWHM(model);     
        case {'difwhmin', 'difwhm'}
            [tmp1, tmp2, tmp3, val, tmp4, tmp5] = rmGetDoGFWHM(model); 
        case {'positivearea', 'positiveauc'}
            [tmp1, tmp2, tmp3, tmp4, val, tmp5] = rmGetDoGFWHM(model);
        case {'totalarea', 'totalauc'}
            [tmp1, tmp2, tmp3, tmp4, tmp5, val] = rmGetDoGFWHM(model);                
        case {'allsigma2stats', 'alldogstats'}
            [tmp1, tmp2, tmp3, tmp4, tmp5, tmp6] = rmGetDoGFWHM(model);
            val=[tmp1;tmp2;tmp3;tmp4;tmp5;tmp6];
        case {'supression', 'supressionindex'} %After Cavanaugh, J.R, Bair, & Movshon 2002
            [tmp1, tmp2, tmp3, tmp4, tmp5, tmp6] = rmGetDoGFWHM(model);
            val=(tmp5-tmp6)./tmp5;   
