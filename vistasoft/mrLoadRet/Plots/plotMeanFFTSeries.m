function data = plotMeanFFTSeries(view,scanNum,plotFlag)
%
%    data = plotMeanFFTSeries(view,[scan],[plotFlag=1])
% 
...
% inplane.
if notDefined('scanNum'),  scanNum = getCurScan(view); end
...
if isequal(view.viewType, 'Flat')
	data = flat2ipPlotMeanTSeries(view, scanNum, 'meanfft');
	return
end

nCycles   = numCycles(view,scanNum);
maxCycles = round(numFrames(view,scanNum)/2); % number of frequencies to plot

% Load up the time series for this view.  Should we check whether it is
% already loaded?
ROIcoords  = getCurROIcoords(view);
tSeriesROI = voxelTSeries(view, ROIcoords, scanNum);
