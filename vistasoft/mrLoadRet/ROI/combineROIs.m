function [view, roi, ok] = combineROIs(view, rois, action, name, color, comments)
% function [view, roi, ok] = combineROIs(view, [rois=dialog], [action='union'], [name], [color='b'], [comments])
%
...
if notDefined('name'),	name = sprintf('ROI%i', length(view.ROIs)+1);	end
if notDefined('action'),	action = 'union';							end
if notDefined('color'),	color = 'b';									end
if notDefined('comments'),	comments = '';								end
if notDefined('rois')
	[rois action name color comments ok] = combineROIsGUI(view);
	if ok==0
...
for r = 1:length(rois)
	rois{r} = tc_roiStruct(view, rois{r});
end
...
% /-------------------------------------------------------------/ %
function [rois, action, name, color, comments, ok] = combineROIsGUI(view);
...
if ~isempty(view.ROIs)
    ...
    for iROI = 1:length(view.ROIs); roiNameList{iROI} = view.ROIs(iROI).name; end;
    defROINum = view.selectedROI;
    defColorNum = findstr(view.ROIs(defROINum).color, colorChar);
