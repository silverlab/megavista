...
%Computes min/average/max properties of fibers in a fibergroup
%
%   sfgProperties = dtiFiberProperties(fg, dt, [roi], [fiberDiameter])
%
% Encapsulates functionality from dtiFiberSummary/dtiFiberSummaryNOGUI:
% Reporting fiber length, numfibers, fa, md, axial d, rd, linearity,
% planarity. Another measure not reported by
% dtiFiberSummary/dtiFiberSummaryNOGUI but computed and reported by current
% dtiFiberProperties function is volume of the fibergroup. 
% 
% If 'roi' parameter is provided, then the fiber properties will
% be computed over the portion of the fiber group limited to that ROI.
% Fiber count (numberOfFibers) will not change. Length reported will refer
% to the average length of the fiber segments limited by the ROI (computed
% using a very crude approximation). 
% 
...
% The value for fiberDiameter may come from dtiCullFibers as Tt+distanceCrit, 
% or from parameters used when running Blue Matter. For the volume measure
% given fiberDiameter to be meaningful, the fiber group is expected to have
% no duplicates (duplicates are defined as two fibers with fiber-to-fiber
% distance (distance metrics vary) less than fiberDiameter). To remove the
% duplicates, use dtiCullFibers or density regularization using Blue
% Matter.
% 
% Output:
% sfgProperties  - a structure with properties for each subgroup of
%                  fibers specified in fg.subgroup field. The first element
%                  of the structure refers to all the fibers (the union of 
%                  the fiber subgroups). The second element refers to the 
%                  first subgroup, etc. The properties reported in the
%                  structure: numberOfFibers, fiberLength, FA, MD, axialADC,
%                  radialADC, linearity, planarity, fiberGroupVolume
% PartialVolumeImg - 4D image with maps of partial volume contribution across
%                    fiber groups (identified with fg.subgroup). If
%                    fiberDiameter was specified, these maps are NOT computed. 
%                    (TODO: think if it makes sense to).
%                    To save ouput partial volume maps as 4D image
...
end %End loop over subgroups
  
% VOLUME COMPUTING 
% Compute the volume of fg.fibers in each subgroup while
% removing partial volume contribution by other fibers in fg. In
% case fiberDiameter is provided we assume the fibers are
% density-regularized (or culled) hence no partial volume
% correction is needed. In case no fiber Diameter is provided, we
% will output maps of partial volume. 
...
if ~isnan(fiberDiameter)
  ...
  partialVolumeImg=NaN; 
    ...
    % recompute volume to properly treat patial volume efect.
    if nfg>1
...
if length(fg)>1 %Fg is already an array at this point, no subgroups
