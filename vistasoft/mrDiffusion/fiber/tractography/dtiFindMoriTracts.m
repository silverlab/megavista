...
%
% By default whole brain tractography is performed and then Mori-classified.
% Otherwise, provide a file name fgFile for a fiber group if willing to Mori-classify an existing fiber group.
%
% Input parameters: 
% fgFile                   - A file with prefiously tracked elsewhere fibers 
%                          to be categorized.
...
% 2009.08.28 EIR added interhemispheric fiber split below ACPC and removed fghandle output (because the parent FG is modified to add split fibers..
% 2009.09.27 ER  added useRoiBasedApproach option which implements Zhang (2008) Neuroimage 42 method.
% 2009.10.2 ER:  no longer need to fit Quench state file labels to 8 grp limit.
...
if useRoiBasedApproach
    ...
    [fgOut,contentiousFibers,  InterHemisphericFibers] = dtiIntersectFibersWithRoi([], 'not', [], midSaggitalRoi, fg); %NOTICE: ~keep3 (not "keep3") will mark fibers that DO NOT cross midSaggitalRoi.
...
if(saveQuench)
    ...
    fgInds = fg.subgroup;
    dtiQuenchSaveFibersState(fgInds, [outFile(1:end-4) '.cst']);
end
