...
% Purpose: Allows you to turn a nifti file which is known to be in MNI space into an ROI viewable in
% mrDiffusion. You want this image to have only a few discrete integer values
% (e.g., a mask of ones, or of 1, 2, 3...N if want to load N rois). 
%
% The code is based on dtiLoadROIsfromNifti(hObject,handles) which assumes
% that the NIFTI ROI was created in the on the same t1.nii.gz that you aligned the raw DTI
% data to in dtiRawPreprocess. Current function, however, adds a step of
% computing bo-to-MNI_EPI alignment. This transformation is applied to the
% loaded NIFTI so that it is now in dt6 space. 
...
%Based on dtiLoadROIsfromNifti, this function will (hopefully) load differently labeled
