...
[bs.permuteMatrix, tmp, nUniqueDirs] = ...
    dtiBootGetPermMatrix(dlmread(dwDir.bvecsFile),dlmread(dwDir.bvalsFile));   %#ok<ASGLU>
...
if size(dwRawAligned.data,4)<14
    if strmatch(dwParams.fitMethod,'ls')
...
switch lower(dwParams.fitMethod)
    case {'ls'}
        dt6FileName = dtiRawFitTensorMex(dwRawAligned, dwDir.alignedBvecsFile,...
            dwDir.alignedBvalsFile, dwParams.dt6BaseName,...
            bs,[], dwParams.fitMethod,[],[],dwParams.clobber);
        
    case {'rt'}
        dt6FileName = dtiRawFitTensorRobust(dwRawAligned, dwDir.alignedBvecsFile,...
            dwDir.alignedBvalsFile, dwParams.dt6BaseName,[],[],[], ... 
...
[pddT1,tmp,mm] = dtiRawCheckTensors(fullfile(dwParams.dt6BaseName,'bin',...
                                  'tensors.nii.gz'),t1FileName); %#ok<ASGLU>
