...
% bvecs- a 3xM list of DW gradient directions where M = the number of ADC
% volumes in the dataset. Again, if this is a string, it is assumed to be a
% filename pointing to an FSL-style bvecs file (space-delimited text file).
%
...
%
% plotType options currently include: 'adcProfile','diffusionEllipsoid'
%
...
% This ellipsoid defines a surface of constant mean-squared
% displacement of the spin-labeled protons at time T. Ie, the 
% mean squared diffusion distance of the protons over time T.
% The length of each of the ellipsoid axes is sqrt(2*lambda_i*T), 
...
if plotType(1) == 'd'
    % diffusion ellipsoid axis lengths = sqrt(2*lambda_i*T); here T=1
    % Scale the eigenvectors in the columns of vec by sqrt(2*lambda_i).
