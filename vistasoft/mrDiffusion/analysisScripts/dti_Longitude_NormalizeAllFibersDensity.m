%script dti_Longitude_NormalizeAllFibersDensity -- normalize the density of
%ALL (not just Mori Groups). 
%ER 05/2009
...
for s=1:size(subjectID, 2); 
    ...
    dtiMakeBrainMaskSafe(fullfile(pwd, 'dti06trilinrt'), dtiRawdir);
    %= Prepare average aligned raw dti data, accompanied by bvals and
    %bvecs. 
    dtiAverageRawAligned(dtiRawdir);
