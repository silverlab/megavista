...
c = (c+1); % Counter
        ...
        if ~isempty(sub) % If there is no data for dtiYr{kk}, skip.
            ...
            % This now looks in the ROIs directory at the top level of the
            % subjects directory. If one is not found it will create one
            % and save it to the dti06/ROIs folder. For years 2-4 there is
            % no CC.mat in sub/ROIs. One must be created. that is done
            % below.
            % if ~exist(fullfile(subDir,'ROIs','CC.mat'));
            ...
            for kk=1:length(ROI1)
                roi1 = fullfile(roiDir,'MT', [ROI1{kk},'.mat']);
                roi2 = fullfile(roiDir,'MT', [ROI2{kk},'.mat']); % For pairs of ROIs use ROI2{kk}
                ...
                % params.roi1Seed     = roi1SeedFlag;
                % params.roi2Seed     = roi2SeedFlag;
                ...
                outFile = ['fg_',projectName,'_',roi1,'_',roi2,'_',timeStamp,'.Bfloat'];
