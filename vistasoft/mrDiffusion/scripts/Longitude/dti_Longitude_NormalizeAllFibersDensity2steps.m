% Use this script to set up blue matter normalization of the full dataset.

% Run one participant: normalize full brain tractography set (not just
% mori). Two-stage procedure: first, generate a mori set and initialize the
% solution. Second, throw in the rest of the fibers and normalize them, too.

% Assumes each dti folder already contains safe brain mask, bvals/bvecs
% averages & contrack parameter files. See
% dti_Longitude_NormalizeMoriFibersDensity.m for an example how to set up a
% batch process to generate those.
...
load('/biac3/wandell4/users/elenary/longitudinal/data/subjectCodes3Adults.mat');  
...
return
%%%%%%%%%%% RUN AFTER ALL TRUESA and VALIDATION JOBS DONE -- this step is
%%%%%%%%%%% not required to continue on with full  brain BM normalization.
%%%%%%%%%%% This step is required for quality checks though.
%%%%%%%%%%%
%Tranform ind to mat
for s=1:size(subjectID, 2)
    cd([project_folder subjectID{s}]);
    ...
    try
        ...
        fprintf( subjectID{s});
        %TrueSA solution are indices of fibers to keep--in the space of
        %allConnectingGM_MoriGroups. PLUS1!!! (those indices count from 0)
...
while a
    ...
    for s=1:size(subjectID, 2)
        %check that the initial guess job at least finished. system(grep)
        %returns "0" if there is a match found---oukey. (system error codes of
        %1 or 2 otherwise).. And also see if maybe this job is already
        %running...
        display([subjectID{s}]);
...
%Final solution -- Tranform ind to mat.  allConnectingGM_DN.mat: Mori
%Labels for the indices correspoinding to first N fibers can be obtained
%from allConnectingGM_MoriGroups, the rest should be "0";
for s=1:size(subjectID, 2)
    cd([project_folder subjectID{s}]);
    dt6File=fullfile('dti06trilinrt', 'dt6.mat');
    ...
    try
        ...
        fprintf( subjectID{s});
        %TrueSA solution are indices of fibers to keep--in the space of
        %allConnectingGM. PLUS1!!! (those indices count from 0)
        DN_ind=textscan(fid, '%d');DN_ind=DN_ind{1}; fclose(fid);
