...
vbaseDir = '/biac2/wandell2/data/anatomy/';
...
vsubs = {'brewer','dougherty'};

% 'wade','aw040809', % Not a binary mat file
...
for ii=1:length(subs)
    % Set Directories
    vSubDir = dir(fullfile(vbaseDir,[vsubs{ii} '*']));
    vAnatDir = fullfile(vbaseDir,vSubDir.name);
    vAnatomy = fullfile(vAnatDir,'vAnatomy.dat');

    % fSubDir = dir(fullfile(fmriDir,[subs{ii} '*MotDisc*']));
    roiDir = fullfile(vAnatDir,'ROIs');
    fprintf(fid,'\n------------------------------------------ \n');
