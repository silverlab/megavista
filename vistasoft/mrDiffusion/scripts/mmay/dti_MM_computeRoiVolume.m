...
logFile = fullfile(logDir,['roiVolume_1k_OccHomDistThresh01_',dateAndTime '.txt']);
fid=fopen(logFile,'w');
...
rois = {'Mori_Occ_CC_100k_top1000_LEFT_hom_ccRoi'...
    'Mori_Occ_CC_100k_top1000_RIGHT_hom_ccRoi'};
    

% scoredFG_Controls_Mori_Occ_CC_100k_top1000_LEFT'...
...
for ii=1:length(subs)
    for jj=1:length(yr)
        ...
        for kk=1:length(rois)
            if exist(fullfile(roiDir,[rois{kk} '.mat']),'file'), disp('Roi found.');

            roi = dtiReadRoi(fullfile(roiDir,[rois{kk} '.mat']));
            s.roiName = roi.name;
