...
% cd(baseDir);
for(ii=1:length(f)) % skip first two directories, which are weird ones
    fname = fullfile(baseDir, f{ii}, dtname{ii}); %concatenates stuff in braket into one string, commas add slashes to create a valid path
