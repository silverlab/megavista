...
%this function take two images and display a  overlay of them.
%GrayIm will be the gray scale image and one will
%overIm overlay the gray image.
% Note that the image HAVE to be in the same size and transformation !!!
%Incmap is the color map that will be used for the overlay image.
% Incmap is [26 3] matrix the default = autumn(256).
%threshold is the values from which the The overlay image will be
%shown default = 0;
...
if (~exist('Incmap','var')||isempty(Incmap)),
    Incmap=autumn(256);
end;

if (~exist('GrayIm','var')||isempty(GrayIm)),
   error('no gray image');
end;
if (~exist('overIm','var')||isempty(overIm)),
   error('no Overlay image');
end;
