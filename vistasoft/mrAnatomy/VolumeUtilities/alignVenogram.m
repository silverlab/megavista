...
% These scans are contrast scans, which means two sets of images are
% acquired. If there are n slices in the scan, you will get 2n dicom
% images. Dicom images [1:n/2-1] comprise a volume that highlights blood
% flow. The second set of dicoms, images [n/2:n], comprise the identical
% volume, but show background flow. Subtracting the second half from the
% first half yields a nice anatomical image that highlights vessels,
% especially venous vessels. In addition to the venogram, we also acquire a
% quick t1 reference anatomy, usually with the same coverage (slice
% presrciption) as the venogram. The venogram and the reference anatomy
% typically only cover the back of the brain.
%
% Instructions:
...
%
% Notation:
% Once have split the 3DPC scan into two directories, we refer to the first
% set of images (or the nifti that the images will become) as the
...
% images from the venogram itself. This set can be used for the alignment). The advantage of using the
% reference antomy  is that it has high contrast and should align well to
% the stored t1. The advantage of using the venogram
