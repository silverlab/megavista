function ecogPlotZERP_GUI(par,bef_win,aft_win,condnames)
...
% modified for z-scored ERP jc apr 2011
...
try % don't crash if file not found -jc
    load(sprintf('%s/ZERP_%s_%s_%s_%s.mat',fn,par.exptname,condition,windurstr,block)); % Zscore
    mean_erp = ERP.elecs(elecs).mean; % zcored
...
plot(mean_erp,lineColor,'LineWidth',2),
