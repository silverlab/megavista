function [peakTimes,halfPeakTimes,peakValues,timeToPassThresh,noRespTrials] = ecogResponseLatencyMultBands(par,ci,freqBand,timeWindow,eventtimes,condnums)
...
locut = [80 90 100 110 120 130 140 80];
hicut = [90 100 110 120 130 140 150 150];
%locut = freqBand(1);
%hicut = freqBand(2);
...
for curBand = 1:length(locut)
    power_tmp= double(amplitude(curBand,:).^2); % Signal Power 
    ...
    for condnum = condnums
        ...
        thresh = 5; % threshold for counting the peak in the mean response time; 2 means a 100% increase, I think. ***********
        ...
        for tri = 1:length(maxVals)
            if maxVals(tri)<thresh  % peak does not pass threshold
                maxVals(tri)=NaN;
                noResp = [noResp tri];
                tpeak(tri) = NaN;
                peakVal(tri) = NaN;
                ...
                aboveThreshTimes = aboveThreshTimes(aboveThreshTimes>0.03); % don't allow before 30 millisecond response
        ...
        noRespTrials{condnum,curBand,:} = noResp;
        peakTimes{condnum,curBand,:} = tpeak;
        halfPeakTimes{condnum,curBand,:} = tHalfMax;
        peakValues{condnum,curBand,:} = peakVal;
        timeToPassThresh{condnum,curBand,:} = timeOverThresh;
